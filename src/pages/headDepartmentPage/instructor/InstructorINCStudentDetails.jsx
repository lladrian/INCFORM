...
const InstructorINCStudentDetails = () => {
  ...
  const [instructorApproveForm, { isLoading }] =
    useInstructorApproveFormMutation();
  ...
  const handleGradeChange = (event) => {
    ...
    // Validate and update state if the value matches the regex and is within the desired range
    if (regex.test(value) && isWithinRange(value)) {
      setGrade(value);
    }
  };

  // Function to check if the number is within the acceptable range
  const isWithinRange = (value) => {
    const numValue = parseFloat(value);
    // Define the maximum acceptable value here
    const maxValue = 10;
    return numValue <= maxValue || isNaN(numValue);
  };

  const approvalSteps = ["Enter Grade", "MFA Verification"];
  ...
  const handleNext = () => {
    if (stepIndex < steps.length - 1) {
      ...
      // Handle final step (Submit) logic here
    }
  };

  const handleSubmit = async () => {
    try {
      const response = await instructorApproveForm({
        ...
        <Card variant="outlined">
          <CardContent>
            ...
            <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
                  ...
                  {stepIndex === 0 ? (
                    <Grid item xs={12}>
                      <TextField
                        ...
                        inputProps={{
                          step: "0.1", // Allows input with up to one decimal place
                          min: "1", // Set a minimum value if needed
                          max: "3", // Set a maximum value if needed
                        }}
              ...
              >
                <Button
                  onClick={stepIndex === 0 ? handleNext : handleSubmit}
                  color="primary"
                  variant="contained"
                  sx={{
                ...
                >
                  {stepIndex === approvalSteps.length - 1 ? "Approve" : "Next"}
                </Button>
                <Button
                  onClick={handleClose}
                  color="secondary"
...
export default InstructorINCStudentDetails;
